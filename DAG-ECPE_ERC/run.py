...
if __name__ == '__main__':
    ...
    parser.add_argument('--iemocaptest',  default=True)
    ...
    wandb_config=dict(lr=args.lr,windowp=args.windowp,batch_size=args.batch_size)
    wandb.init(config=wandb_config,reinit=True,project='erc_dagat_vae_test',name='adj-1_lr_{}_batch_{}'.format(args.lr,args.batch_size),settings=wandb.Settings(start_method="fork"))
    ...
    logger = get_logger(path + args.dataset_name + '/'+str(args.gnn_layers)+str(args.batch_size)+str(args.dropout)+str(args.lr)+str(args.withvae)+'logging.log')
    ...
    if args.dataset_name=='DailyDialog':
        ...
        wandb.log('Best micro/macro F-Score based on test:{}/{}'.format(all_fscore[0][1],all_fscore[0][3]))
    else:
        logger.info('Best F-Score based on validation:{}'.format(all_fscore[0][1]))
        logger.info('Best F-Score based on test:{}'.format(max([f[1] for f in all_fscore])))
        wandb.log('Best F-Score based on test:{}'.format(max([f[1] for f in all_fscore])))
