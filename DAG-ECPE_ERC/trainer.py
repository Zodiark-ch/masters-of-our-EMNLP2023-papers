...
def train_or_eval_model(model, loss_function, dataloader,epoch, cuda, args, optimizer=None, train=False):
    ...
    for data in dataloader:
        ...
        # print(speakers)
        log_prob_2,e,s = model(features, adj, s_mask, s_mask_onehot, lengths) # (B, N, C)
        # print(label)
        loss1 = loss_function(log_prob_2.permute(0,2,1), label)
        loss_KL=model.loss_KL(e,s)
        #loss_KL=model.loss_vae(log_prob_1, log_prob_2)
        loss=loss1+loss_KL

        label = label.cpu().numpy().tolist()
        pred = torch.argmax(log_prob_2, dim = 2).cpu().numpy().tolist()
        preds += pred
        labels += label
