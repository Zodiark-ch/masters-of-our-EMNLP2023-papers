...
class CAVAE_dagnn(nn.Module):
    ...
    def forward(self,features,adj,s_mask,s_mask_onehot,lengths):
        fu,_,adje,adjs=self.encoder(features,adj,s_mask,s_mask_onehot,lengths)
        #fu,s=self.encoder(features,adj,s_mask,s_mask_onehot,lengths)
        z=self.sampling(adje,adjs) 
        X,b_inv=self.decoder(fu,adj,s_mask,s_mask_onehot,lengths,z)
...
class DAGNNencoder(nn.Module):
    def __init__(self,args):
        ...
        layers += [self.dropout]
        layers += [nn.Linear(args.gnn_hidden_dim, args.gnn_hidden_dim)]
    ...
    def forward(self,features,adj,s_mask,s_mask_onehot,lengths):
        ...
        adje=torch.zeros([features.size()[0],1,num_utter]).cuda()
        adje_list=[adje]
        
        adjs=torch.zeros([features.size()[0],1,num_utter]).cuda()
        adjs_list=[adjs]
        for l in range(self.args.gnn_layers):
            C = self.grus_c[l](H[l][:,0,:]).unsqueeze(1) #H
            ...
            for i in range(1, num_utter):
                ...
                B, M,e,s= self.gather[l](H[l][:,i,:], H1, H1, adj[:,i,:i], s_mask[:,i,:i])
                B=nn.ZeroPad2d(padding=(0,num_utter-B.size()[-1],0,0))(B.squeeze(1)).unsqueeze(1)
                e=nn.ZeroPad2d(padding=(0,num_utter-e.size()[-1],0,0))(e.squeeze(1)).unsqueeze(1)
                s=nn.ZeroPad2d(padding=(0,num_utter-s.size()[-1],0,0))(s.squeeze(1)).unsqueeze(1)
                ...
                if i==0:
                    ...
                    adje=torch.cat((adje,e),dim=1)
                    adjs=torch.cat((adjs,s),dim=1)
                # print('H1', H1.size())
                # print('----------------------------------------------------')
            H.append(H1)# H=[tensor[H],tensor[H1],……]
            adjB_list.append(adjB)
            adje_list.append(adje)
        ...
        return logits,adjB_list,adje_list,adjs_list
...
class DAGNNdecoder(nn.Module):
    ...
    def forward(self,features,adj,s_mask,s_mask_onehot,lengths,adjB_list):
        ...
        H0 = self.dropout(F.relu(features))
        H = [H0]##
        for l in range(self.args.gnn_layers):
